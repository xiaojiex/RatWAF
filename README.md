...
CREATE TABLE `waf_log` (
  ...
  `full_url` text NOT NULL COMMENT 'URL',
  PRIMARY KEY (`id`)
...
```go
dsn := "user:pass@tcp(127.0.0.1:3306)/ratwaf?charset=utf8mb4&parseTime=True&loc=Local"
```
...
反向代理入口：`http://127\.0\.0\.1:8081`（转发至本地业务服务 8080 端口）
